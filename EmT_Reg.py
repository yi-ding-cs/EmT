...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.to_qkv = nn.Linear(dim, dim_head, bias=False)

        self.token_mixer = nn.GRU(input_size=dim_head, hidden_size=dim_head, num_layers=2,
                                  batch_first=True, bidirectional=True, dropout=0.5 * dropout)

        self.to_out = nn.Sequential(
            nn.Linear(2*dim_head, dim),
            nn.Dropout(dropout)
        ) if project_out else nn.Identity()

    def forward(self, x):
        out = self.to_qkv(x)
        out, _ = self.token_mixer(out)
...
class EmT(nn.Module):
    def __init__(self, layers_graph=[1, 2], layers_transformer=1, num_adj=3, num_chan=62,
                 ...
                 dropout=0.25, num_class=3, graph2token='Linear', encoder_type='GCN'):
        ...
        self.feature_before = None
        self.feature_after = None
    ...
    def forward(self, x):
        ...
        x_ = x.contiguous().view(x.size(0), -1)
        ...
        self.feature_before = x
        x = self.transformer(x)
        self.feature_after = x
        x = self.MLP(x)
        return x
...
if __name__ == "__main__":
    data = torch.ones((2, 96, 32, 6))
    emt = EmT(layers_graph=[1, 2], layers_transformer=2, num_adj=3, num_chan=32, num_feature=6, hidden_graph=32,
              K=3, num_head=32, dim_head=32, dropout=0.5, num_class=1, graph2token='Linear', encoder_type='Cheby')
    print(emt)
