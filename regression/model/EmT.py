...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.lstm = nn.GRU(input_size=dim_head, hidden_size=dim_head, num_layers=2,
...
if __name__ == "__main__":
    data = torch.ones((16, 8, 62, 5))
    emt = EmT(layers_graph=[1, 2], layers_transformer=2, num_adj=3, num_chan=62, num_feature=5, hidden_graph=16,
              K=2, num_head=8, dim_head=16, dropout=0.25, num_class=3, graph2token='Linear', encoder_type='Cheby')
    print(emt)
